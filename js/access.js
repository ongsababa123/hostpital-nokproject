...
function edit(i) {
    act = "update";
    $("#accessCode").val(access[i].accessCode);
    $("#accessName").val(access[i].accessName);
    $("#accessCode").prop("readonly", true);
    $("#accessName").prop("readonly", false);
    $("#btnSave").prop("disabled", false);
    $("#accessName").focus();
}

function del(accessCode) {
    if (confirm('ต้องการลบ ? ')) {
        $.post(prg, { accessCode, act: 'delete' }, function(data, access) {
            display('read');
            alert(data)
        })
    }
...
function add() {
    ...
    $("#accessName").val("");
    $("#accessCode").focus();
    $("#accessCode").prop("readonly", false);
    $("#accessName").prop("readonly", false);
    $("#btnSave").prop("disabled", false);
}