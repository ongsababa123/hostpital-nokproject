    ...
    $.post(shiftSchedule_prg, {
        ...
        $.each(respone, function(i, item) {
            ...
            switch (item.shift_type) {
                case 'M':
                    {
                        str += `<div class="mt-1 px-4 py-1 text-white rounded bg-primary">เวรเช้า</div>`;
                        break;
                    }
                case 'A':
                    {
                        str += `<div class="mt-1 px-4 py-1 text-white rounded bg-warning">เวรบ่าย</div>`;
                        break;
                    }
                case 'N':
                    {
                        str += `<div class="mt-1 px-4 py-1 text-white rounded bg-danger">เวรดึก</div>`;
                        break;
                    }
                default:
                    {
                        str += `<div class="mt-1 px-4 py-1 text-white rounded bg-success">เวร 24 ชม.</div>`;
                        break;
                    }
            }
